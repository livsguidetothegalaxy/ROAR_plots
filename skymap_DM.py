...
sc = ax.scatter(np.array(lll_known)*np.pi/180., np.array(bbb_known)*np.pi/180., c=DM_known, marker= ".", label= "Known Pulsars")
sc = ax.scatter(np.array(lll_cand)*np.pi/180., np.array(bbb_cand)*np.pi/180., c=DM_cand, marker= "*", label = "Candidates")
